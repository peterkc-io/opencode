...
describe("tui thread", () => {
  ...
  test("forwards the CLI environment to the TUI worker", async () => {
    ...
    expect(source).toContain("Object.entries(process.env)")
    expect(source).toContain("[INSTANCE_ID_ENV]: instanceID")
    expect(source).toContain('[COMPONENT_ENV]: "server-worker"')
  })

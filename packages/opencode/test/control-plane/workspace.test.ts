  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const previous = workspaceInfo(instance.project.id, previousType)
        yield* insertWorkspace(previous)
        registerAdapter(instance.project.id, previousType, localAdapter(path.join(test.directory, previousType)).adapter)
        const foreign = yield* provideTmpdirInstance(
          () => SessionNs.Service.use((sessions) => sessions.create({})),
          { git: true },
        )
